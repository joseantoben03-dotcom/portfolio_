...
const animatedElements = document.querySelectorAll('.skill-category, .project-card, .cert-card, .timeline-item, .stat');
...
timelineItems.forEach(item => {
    ...
    if (item.querySelector('.timeline-content')) {
        item.style.transform = item.classList.contains('odd') ? 'translateX(50px)' : 'translateX(-50px)';
    }
...
console.log('%cLooking at the code? Feel free to reach out!', 'color: #8b5cf6; font-size: 14px;');

document.querySelectorAll('.project-link, .social-links a').forEach(link => {
    link.addEventListener('click', (e) => {
        if (link.getAttribute('href') === '#') {
            e.preventDefault();
            alert('Please update this link with your actual GitHub/Demo/Social media URL!');
        }
    });
});
